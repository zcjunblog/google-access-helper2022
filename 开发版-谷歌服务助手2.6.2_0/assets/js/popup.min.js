...
function load_page(){send_message('reload',function(local){get('/popup',show_popup,show_error);});}
function show_popup(data){if(data.hasOwnProperty('html')){$('body').html(data.html);handle_form();}
if(data.hasOwnProperty('session')){chrome.runtime.sendMessage({'action':'update','session':data.session},function(local){get('/check',function(html){$('#checking').html(html);});});}
if(data.hasOwnProperty('open_url')){window.open(data.open_url,'ghelper');}
if(data.hasOwnProperty('location')){window.location=data.location;}}
function handle_form(){$(".switchsmall").bootstrapSwitch();$('.switchsmall').on('switchChange.bootstrapSwitch',post_popup);$('select').on('change',post_popup);}
function post_popup(){$('#pipup-event').html('<img width="20px" src="assets/img/loading.gif" />');var url=$('form').attr('action');var data=$('form').serialize();post(url,data,show_popup,show_error);}
load_page();